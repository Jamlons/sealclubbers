 ...
 * @param {Integer} account_id - Account ID of the player
 * @returns {Promise<{status: string, message: string|Integer}>} Returns account_id on success, error on failure.
 */
export default async function fetchAndSaveUserBattleData(account_id) {
  try {
    ...
    const tanksData = tankResponse.data.data[account_id];
